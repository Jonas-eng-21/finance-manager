...
class CreateTransactionUseCaseTest {
    ...
    void should_create_transaction_successfully() {
        ...
        CreateTransactionDTO dto = new CreateTransactionDTO(
                userId, TransactionType.INCOME, new BigDecimal("5000.00"), categoryId, "Salário do mês", LocalDate.now()
        );
    ...
    void should_fail_when_category_not_found() {
        CreateTransactionDTO dto = new CreateTransactionDTO(
                1L, TransactionType.EXPENSE, new BigDecimal("100.0"), 99L, "Teste", LocalDate.now()
        );
    ...
    void should_fail_when_category_belongs_to_another_user() {
        Long intruderId = 2L;
        CreateTransactionDTO dto = new CreateTransactionDTO(
                intruderId, TransactionType.EXPENSE, new BigDecimal("100.0"), 10L, "Teste", LocalDate.now()
        );
    ...
    void should_fail_when_category_type_mismatch() {
        ...
        CreateTransactionDTO dto = new CreateTransactionDTO(
                userId, TransactionType.EXPENSE, new BigDecimal("100.0"), 10L, "Teste", LocalDate.now()
        );
